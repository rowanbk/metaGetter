...
var fetchEventInfo = function(eventId, callback) {
	req('http://mtgtop8.com/event?e='+eventId, function(err, res) {
		...
		var players;
		var date;
		if($('table div table td[align=center] div').length < 2){
			callback(null, null)
		}
		console.log($('table div table td[align=center] div'))
...
var fetchLegacyEvents = function(page, callback) {
	...
	req.post('http://mtgtop8.com/format?f=LE&meta=16', { form:{ cp:page } }, function(err, res) {
		if (err) return callback(err);
